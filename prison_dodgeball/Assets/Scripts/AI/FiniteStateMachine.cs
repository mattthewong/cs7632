    ...
    public class FiniteStateMachine<T> : IFiniteStateMachine<T>
    {
        ...
        public void Update()
        {
            ...
            do
            {
                ...
                if (updateOnEnter && numUpdates >= maxUpdates)
                    Debug.LogError($"Number of updates on transition has reached limit ({numUpdates})");

            } while (updateOnEnter && numUpdates < maxUpdates);
        }
