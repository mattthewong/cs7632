...
{
    public const string StudentName = "Matthew Wong";
        ...
        out float altT)
    {
        ...
        var pt0 = new Vector3(targetInitPos.x, targetInitPos.y, targetInitPos.z);
        var pp0 = new Vector3(projectilePos.x, projectilePos.y, projectilePos.z);
        // a vector
        Vector3 a = (pp0 - pt0); // boldface a
        interceptT = a.magnitude / maxProjectileSpeed;
        altT = -1f;

        Vector3 vt = targetConstVel;
        if (targetConstVel.x == 0 && targetConstVel.y == 0 && targetConstVel.z == 0)
        {
            projectileDir = targetForwardDir;
        }
        else
        {
            projectileDir = vt;
        ...
        float sT = vt.magnitude; // speed of target
        float sP = maxProjectileSpeed; // speed of projectile (assumed to be max projectile speed)

        float magA = a.magnitude; // non-bold a
        Vector3 aNorm = Vector3.Normalize(a); // normalized unit vector A
        Vector3 vTNorm = Vector3.Normalize(vt); // normalized unit vector vT
        float cosTheta = Vector3.Dot(aNorm, vTNorm); // solve for cosine of theta by using unit vectors A and vT

        // apply quadratic formula to solve for intercept T
        float innerArg1 = Mathf.Pow(2 * magA * sT * cosTheta, 2);
        float speedDiff = Mathf.Pow(sP, 2) - Mathf.Pow(sT, 2);
        float innerArg2 = 4 * speedDiff * Mathf.Pow(magA, 2);
        float divisor = 2 * speedDiff;
        float radicand = innerArg1 + innerArg2;
        float sqrt = Mathf.Sqrt(radicand);
        float outer = - (2 * magA * sT * cosTheta);        
        if (radicand < 0 || divisor == 0)
        {
            return false;
        }


        float tc1 = (outer + sqrt) / divisor;
        float tc2 = (outer - sqrt) / divisor;
        ...
        projectileDir = Vector3.Normalize(((pt0 - pp0) / interceptT) + vt - (0.5f*projectileGravity* interceptT)); // vp

        if (projectileDir.magnitude > maxProjectileSpeed)
        {
            return false;
        }
        Vector3 targetDir1 = Vector3.Normalize(((pt0 - pp0) / interceptT) + vt - (0.5f * targetConstVel * interceptT));

        Vector3 newPos = Vector3.Normalize(pp0 + (projectileDir * interceptT) + (0.5f * projectileGravity * Mathf.Pow(interceptT, 2)));
        Vector3 targetDir = Vector3.Normalize(pt0 + (projectileDir * interceptT) + (0.5f * targetConstVel * Mathf.Pow(interceptT, 2)));
        Vector3 dist1 = projectileDir - targetDir1;
        Vector3 dist2 = newPos - targetDir;
        Vector3 diff = dist1 - dist2;
        //projectileSpeed = (dist.magnitude / interceptT);
        //Debug.Log(maxAllowedErrorDist + " " + dist.magnitude);
        if (dist1.magnitude > maxAllowedErrorDist)
        {
            return false;
        }
    ...
    abstract class MinionStateBase
    {
        ...
        protected bool FindClosestAvailableDodgeball(out PrisonDodgeballManager.DodgeballInfo dodgeballInfo)
        {
            ...
            // ADDITION: mark this to prevent other minions from getting it
    ...
    class GoToThrowSpotState : MinionState
    {
        ...
        {
            base.Enter();
            // Addition: Immediately throw if the nearest opponent is within a distance of 6 from the minion
            Vector3 border = Mgr.TeamAdvance(Team).position;
            if (Mgr.FindClosestNonPrisonerOpponentIndex(Minion.transform.position, Team, out opponentIndex))
            {
                ...
                {
                    if (Vector3.Distance(opponentInfo.Pos, Minion.transform.position) < 6)
                    {
                        // automatically throw here.
                    } else
                    {
                        Minion.GoTo(border);
                    }
    ...
    class RescueState : MinionState<MinionScript>
    {
        ...
        {
            DeferredStateTransitionBase<MinionFSMData> ret = null;
            PrisonDodgeballManager.OpponentInfo[] opponentInfo = new PrisonDodgeballManager.OpponentInfo[Mgr.TeamSize];
            PrisonDodgeballManager.OpponentInfo[] teamInfo = new PrisonDodgeballManager.OpponentInfo[Mgr.TeamSize];
            // just in case something bad happened
            if (!Minion.HasBall)
            {
                return CollectBallTransition;
            ...
            // ADDITION:
            // if there two or less non-prisoner opponents left and 3 or more non-prisoner teammates left, just attack
            if (Mgr.GetAllOpponentInfo(Team, ref opponentInfo))
            {
                int oppPrisonerCount = 0;
                int teamPrisonerCount = 0;
                foreach (var opp in opponentInfo)
                {
                    if (opp.IsPrisoner)
                    {
                        oppPrisonerCount += 1;
                ...
                PrisonDodgeballManager.Team opTeam;
                if (Team == PrisonDodgeballManager.Team.TeamA)
                {
                    opTeam = PrisonDodgeballManager.Team.TeamB;
                } else
                {
                    opTeam = PrisonDodgeballManager.Team.TeamA;
                }
                Mgr.GetAllOpponentInfo(opTeam, ref teamInfo);
                foreach (var min in teamInfo)
                {
                    if (min.IsPrisoner)
                    {
                        teamPrisonerCount += 1;
                ...
                // if there two or less non-prisoner opponents left and 3 or more non-prisoner teammates left, just attack
                if (opponentInfo.Length-2 <= oppPrisonerCount && teamInfo.Length-3 >= teamPrisonerCount)
                {
                    //Debug.Log("INVOKING ATTACK OVER RESCUE...");
    ...
    class DefensiveDemoState : MinionState
    {
        ...
        {
            var r = Minion.GoTo(Mgr.TeamHome(Team).position + 6f * (new Vector3(Random.value, 0, Random.value)));
        ...
        public override void Enter()
        {
            ...
            evadeWaitTimeSec = 0f * Minion.EvadeCoolDownTimeSec + 0.1f;
        }
